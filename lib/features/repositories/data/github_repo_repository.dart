...
class GithubRepoRepository {
  ...
  Stream<List<GithubRepo>> watchUserRepos(String username) {
    return _isar.githubRepoEntitys
        ...
        .map((entities) {
      return entities.map((entity) => GithubRepo(
        id: entity.githubId,
        name: entity.name,
  ...
  Future<void> syncUserRepos(String username, {int page = 1, int perPage = 30}) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final entities = repos.map((repo) => GithubRepoEntity()
          ..githubId = repo.id
          ..name = repo.name
